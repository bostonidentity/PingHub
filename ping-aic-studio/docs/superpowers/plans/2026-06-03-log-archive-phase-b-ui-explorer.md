# Log Archive — Phase B-ui (Explorer Panel) Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** A "Log explorer" tab that queries the local archive with structured filters and shows a paginated results table with per-row payload drill-down.

**Architecture:** A client `LogExplorePanel` posts filters to `POST /api/logs/archive/query` (Phase B1), renders `{ total, rows, capped }` as a table with Prev/Next pagination and an expandable row payload, and surfaces the `capped` flag. Wired as a fourth tab on the Report page. No new server code — it consumes the B1 query API.

**Tech Stack:** React client component, Next.js, TypeScript. Builds on B1 (`/api/logs/archive/query`, `ArchiveQueryResult`/`ArchiveQueryRow`) and A2c conventions (`DEFAULT_LOG_SOURCES`, datetime controls).

**Reference spec:** `docs/superpowers/specs/2026-06-03-log-archive-design.md` (explore UX = "Structured filters + reports").
**Builds on:** B1 query engine + API.

---

## Testing note

React client component — verified by `tsc --noEmit` + `eslint` + a production `next build` + manual smoke (repo convention: `.tsx` excluded from Vitest). The query API it drives is unit-tested in B1.

---

## File Structure

- `src/app/analyze/LogExplorePanel.tsx` (CREATE) — the explorer (filters, run, table, pagination, drill-down).
- `src/app/analyze/ReportTabs.tsx` (MODIFY) — add a fourth "Log explorer" tab.
- `src/app/analyze/page.tsx` (MODIFY) — pass the new panel.

---

## Task 1: `LogExplorePanel`

**Files:**
- Create: `src/app/analyze/LogExplorePanel.tsx`

- [ ] **Step 1: Create the component** — `src/app/analyze/LogExplorePanel.tsx`:
...
```bash
npx tsc --noEmit
npx eslint src/app/analyze/LogExplorePanel.tsx
```
Expected: `tsc` clean; eslint clean.

Note: `import type { ArchiveQueryResult, ArchiveQueryRow }` is a TYPE-ONLY import — it's erased at compile time, so the server-only `log-query.ts` (which imports `fs`/`better-sqlite3`) is NOT bundled into the client. Verify there is no runtime import from `@/lib/logs/log-query`.

- [ ] **Step 3: Commit**

```bash
git add src/app/analyze/LogExplorePanel.tsx
git commit -m "feat(logs): LogExplorePanel — filtered, paginated archive explorer with payload drill-down
...
## Task 2: Wire the fourth Report tab
...
```
And pass the new prop to `<ReportTabs>` (alongside the existing props):
```tsx
...
npx next build
```
Expected: `tsc` clean; eslint clean (pre-existing AnalyzePanel.tsx warnings excepted — not in scope); full Vitest suite green; `next build` succeeds.

- [ ] **Step 4: Commit**

```bash
git add src/app/analyze/ReportTabs.tsx src/app/analyze/page.tsx
git commit -m "feat(logs): add Log explorer tab to the Report page
...
## Done criteria for Phase B-ui

- A "Log explorer" tab sits between "Log archive" and "ESV orphans".
- It queries `/api/logs/archive/query` with env + window + sources + event/transaction/user/level/free-text filters; Search resets to page 1.
- Results render as a table; Prev/Next page through `total`; a `capped` banner shows when matches exceed the scan cap.
- Clicking a row shows its pretty-printed payload; close dismisses it.
- `tsc --noEmit` + `eslint` clean; full Vitest suite green; `next build` succeeds.

## Manual smoke test

1. Report → Log explorer. Pick `prod`, a window you've pulled, leave sources at all (or just `am-authentication`).
2. Search → table fills; filter by Level=ERROR or an exact eventName → results narrow.
3. Click a row → payload JSON appears; Next/Prev page; confirm "page X of Y" + total.
4. A broad unfiltered multi-day query shows the capped banner.

## Self-review notes (author)

- **Spec coverage:** structured filters (source/time/transaction/event/user/level/free-text) + paginated results table + row payload drill-down + capped surfacing — completes the explore half ("both A and B").
- **Type safety:** `import type` from `@/lib/logs/log-query` keeps the server-only module out of the client bundle while reusing `ArchiveQueryResult`/`ArchiveQueryRow`.
- **Placeholder scan:** none — full component + exact wiring.
- **Consistency:** mirrors JourneyHistoryPanel/LogArchivePanel controls; tabs extended without disturbing existing panels (default stays "journeys").
- **Testing deviation:** UI verified by tsc/eslint + `next build` + manual smoke (repo `.tsx` convention); the query API is unit-tested in B1.
- **Known limits:** exact-match filters for event/transaction/user (free-text covers substring); PAGE fixed at 100; deep paging past the cap unavailable (banner explains). All acceptable for v1.
```
