...
describe("promotion-selection helpers", () => {
  ...
  it("keys directory-based generic scopes by folder name when remapping IDs", () => {
    ...
    writeJson(path.join(scopeDir, "tenantEmailOtpCopy", "tenantEmailOtpCopy.json"), {
      _id: "source-copy-id",
      name: "tenantEmailOtp",
    });
    const logs: string[] = [];

    expect(buildNameToIdMap([scopeDir], "email-templates")).toEqual(new Map([["tenantEmailOtpCopy", "source-copy-id"]]));

    remapIds(
      [scopeDir],
      "email-templates",
      new Map([["tenantEmailOtpCopy", "source-copy-id"]]),
      new Map([["tenantEmailOtpCopy", "target-copy-id"]]),
      logs,
    );

    expect(JSON.parse(fs.readFileSync(path.join(scopeDir, "tenantEmailOtpCopy", "tenantEmailOtpCopy.json"), "utf-8"))).toMatchObject({
      _id: "target-copy-id",
      name: "tenantEmailOtp",
    });
  });
