...
## Task 20: Acceptance — UAT `alpha_kyid*` validation pull

**Files:** none (acceptance test against real UAT tenant)

- [ ] **Step 1: Identify the `alpha_kyid*` types to pull.**

Start the dev server: `cd aic-pipeline && npm run dev`. Navigate to `/data/pull?env=uat`. Look at the type list (left panel / selectable list). Note every type whose name begins with `alpha_kyid` (e.g. `alpha_kyid_access`, `alpha_kyid_role`, etc.).
...
In `/data/pull?env=uat`, click Probe counts on the selected `alpha_kyid*` types. Note which return real numbers vs. which return "unknown". This is informational — pulls don't depend on counts being known.

- [ ] **Step 4: Pull all `alpha_kyid*` types.**

Select all `alpha_kyid*` types in the type list and click Start. Watch the JobCard for progress. Allow the pull to run to completion.
...
In the Data tab, browse each pulled `alpha_kyid*` type. Click into 2–3 records on each. Search for a known field value. Refs lookups should resolve outgoing/incoming refs without error.
