...
A concrete trigger case: in the UAT tenant, `alpha_kyid_access` has more than 200,000 records and the tenant rejects `_countPolicy` with HTTP 400 ("Unrecognized request parameter"). The count probe gives up at the 200,000 pagination cap and the UI shows "unknown." The pull itself isn't capped, but the pull has the same fragility issues plus no progress denominator.
...
After implementation lands, run a real pull in UAT for **all managed-object types whose name starts with `alpha_kyid`**. The original symptom that motivated this work was `alpha_kyid_access` showing "unknown" and being un-pullable in any practical sense. Acceptance is:

- Every `alpha_kyid*` type in UAT pulls to completion.
