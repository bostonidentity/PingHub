...
describe("log-query", () => {
    ...
    describe("&& operator", () => {
        ...
        it("treats whitespace as implicit AND", () => {
            const q = parseQuery("sync request content", ci);
            expect(q.test("sync request content payload")).toBe(true);
            expect(q.test("sync content")).toBe(false);
            expect(q.test("request only")).toBe(false);
            expect(q.highlightTerms).toEqual(["sync", "request", "content"]);
        });

        it("mixes explicit && with implicit AND", () => {
            const q = parseQuery("foo && bar baz", ci);
            // Equivalent to: foo && bar && baz
            expect(q.test("foo bar baz qux")).toBe(true);
            expect(q.test("foo bar")).toBe(false);
        });

        it("implicit AND combines with parentheses and OR", () => {
            const q = parseQuery("(error warn) || debug", ci);
            // (error AND warn) OR debug
            expect(q.test("error and warn together")).toBe(true);
            expect(q.test("debug only")).toBe(true);
            expect(q.test("error only")).toBe(false);
        });
    });
